    ...
    <div className="rounded-2xl border border-border bg-card p-6 shadow-sm relative overflow-hidden transition-all duration-300 hover:shadow-md">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1.5">
              ...
              {user.regNo && (
                <span className="font-mono bg-muted/60 px-1.5 py-0.5 rounded text-[10px] text-foreground">
                  Reg No: {user.regNo}
                </span>
              )}
            </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-5 border-b border-border/60 text-xs">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <p className="font-medium text-foreground flex items-center gap-1.5 mt-0.5">
            ...
            <span>
              {user.team
                ? `${user.team.name} (${user.team.groupLevel || 'Level N/A'})`
                : 'No Team Assigned'}
            </span>
          </p>
        </div>
        ...
        <div className="py-2.5 px-4 mt-4 bg-indigo-500/5 rounded-xl border border-indigo-500/10 flex justify-between items-center text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground font-medium">
              Team Leaderboard Standing ({user.team.groupCode})
            </span>
          </div>
          <span className="font-bold text-indigo-600">
            Rank #{user.team.ranking.rank} ({user.team.ranking.totalPoints} total points)
          </span>
        </div>
      )}
      ...
      <div className="mt-5">
        <h4 className="text-xs uppercase font-bold text-muted-foreground tracking-wider mb-3">
          Skills Roster (Real-time DB)
        </h4>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Primary */}
          <div>
            <span className="text-[9px] uppercase font-semibold text-primary tracking-widest block mb-2">
              Primary Skills
            </span>
            <div className="space-y-2">
              {primarySkills.map((s: any) => (
                <div
                  key={s.skillName}
                  className="bg-primary/5 border border-primary/10 rounded-xl p-2.5 flex flex-col gap-1 transition-colors hover:bg-primary/10"
                >
                  ...
                  <div className="flex justify-between items-center text-[9px] text-muted-foreground mt-0.5">
                    {s.skillRank ? (
                      <span>
                        Rank #{s.skillRank} / {s.totalRanks || 3221}
                      </span>
                    ) : (
                      <span>Unranked</span>
                    )}
                    {s.totalPoints > 0 && (
                      <span className="font-bold text-primary">{s.totalPoints} RP</span>
                    )}
                  </div>
                </div>
              ))}
          ...
          {/* Secondary */}
          <div>
            <span className="text-[9px] uppercase font-semibold text-emerald-600 tracking-widest block mb-2">
              Secondary Skills
            </span>
            <div className="space-y-2">
              {secondarySkills.map((s: any) => (
                <div
                  key={s.skillName}
                  className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-2.5 flex flex-col gap-1 transition-colors hover:bg-emerald-500/10"
                >
                  <span className="text-xs font-semibold text-foreground">{s.skillName}</span>
                  <div className="flex justify-between items-center text-[9px] text-muted-foreground mt-0.5">
                    {s.skillRank ? (
                    ...
                    {s.totalPoints > 0 && (
                      <span className="font-bold text-emerald-600">{s.totalPoints} RP</span>
                    )}
                  </div>
                </div>
              ))}
          ...
          {/* Specialization */}
          <div>
            <span className="text-[9px] uppercase font-semibold text-amber-600 tracking-widest block mb-2">
              Specialization
            </span>
            <div className="space-y-2">
              {specSkills.map((s: any) => (
                <div
                  key={s.skillName}
                  className="bg-amber-500/5 border border-amber-500/10 rounded-xl p-2.5 flex flex-col gap-1 transition-colors hover:bg-amber-500/10"
                >
                  <span className="text-xs font-semibold text-foreground">{s.skillName}</span>
                  <div className="flex justify-between items-center text-[9px] text-muted-foreground mt-0.5">
                    {s.skillRank ? (
                    ...
                    {s.totalPoints > 0 && (
                      <span className="font-bold text-amber-600">{s.totalPoints} RP</span>
                    )}
                  </div>
                </div>
              ))}
      ...
      <div className="mt-6">
        <GitHubAnalyticsPanel username={(user as any).githubUsername} />
      </div>
    </div>
  );
};
