...
import { renderMessageContent, QUICK_PROMPTS, WELCOME_MESSAGE, type Message } from './aiResponse';
...
export const ChatDock: React.FC = () => {
  ...
  const resetConversation = () => {
    ...
    const welcome: Message[] = [
      { id: '0', role: 'assistant', content: WELCOME_MESSAGE, timestamp: new Date() },
    ];
    setMessages(welcome);
        ...
        switch (evt.event) {
          ...
          case 'state.refresh': {
            ...
            window.dispatchEvent(
              new CustomEvent('pv:refresh', { detail: { type: (evt as any).type } }),
            );
            break;
          }
          ...
          case 'tool.progress': {
            ...
            setBrowserSteps((prev) => {
              const idx = prev.findIndex(
                (s) => s.step_type === step_type && s.target === target && s.status === 'running',
              );
              if (idx >= 0) {
                const next = [...prev];
      ...
      <div className="flex items-center gap-2.5 px-4 h-16 border-b border-border shrink-0">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[11px] text-success flex items-center gap-1 leading-tight">
            <span className="h-1.5 w-1.5 rounded-full bg-success inline-block" /> Online · Agent
            ready
          </p>
        </div>
          ...
          <div className="flex flex-col gap-1 pl-9">
            ...
            {browserSteps.map((step, idx) => (
              ...
              >
                ...
                {step.status === 'running' && (
                  <span className="ml-auto h-1.5 w-1.5 rounded-full bg-violet-400 animate-ping" />
                )}
              </div>
            ))}
          </div>
