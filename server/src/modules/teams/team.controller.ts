...
function puid(req: Request): string { return req.params['userId'] as string; }
function pinviteId(req: Request): string { return req.params['inviteId'] as string; }
function ptaskId(req: Request): string { return req.params['taskId'] as string; }
function pcollabId(req: Request): string { return req.params['collabId'] as string; }
...
export const teamController = {
  ...
  async getTeamById(req: Request, res: Response) {
    try {
      const user = req.user;
      if (!user || !user.organizationId) return res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Unauthorized' });
      ...
      console.error(error);
      res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: 'Failed to fetch team details' });
    }
  },
  ...
  async updateTeam(req: Request, res: Response) {
    try {
      const user = req.user;
      if (!user || !user.organizationId) return res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Unauthorized' });
      const team = await teamService.updateTeam(user.organizationId, pid(req), user.id, user.role, req.body);
      res.json(team);
    } catch (error: any) {
      console.error(error);
      res.status(StatusCodes.BAD_REQUEST).json({ message: error.message || 'Failed to update team' });
    }
  },
  ...
  async addMember(req: Request, res: Response) {
    try {
      ...
      const member = await teamService.addMember(user.organizationId, pid(req), userId, roleLabel || 'Member');
      res.json(member);
    } catch (error) {
      console.error(error);
  ...
  async removeMember(req: Request, res: Response) {
    try {
      const user = req.user;
      if (!user || !user.organizationId) return res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Unauthorized' });
      const result = await teamService.removeMember(user.organizationId, pid(req), puid(req), user.id, user.role);
      res.json(result);
    } catch (error: any) {
      console.error(error);
      res.status(StatusCodes.BAD_REQUEST).json({ message: error.message || 'Failed to remove member' });
    }
  },
  ...
  async getCoordinationMetrics(req: Request, res: Response) {
    try {
      ...
      console.error(error);
      res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: 'Failed to compute coordination metrics' });
    }
  },
  ...
  async getAIInsights(req: Request, res: Response) {
    try {
      const user = req.user;
      if (!user || !user.organizationId || !user.id) return res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Unauthorized' });
      const result = await teamService.getAIInsights(user.organizationId, pid(req), user.id);
      res.json(result);
    } catch (error) {
      console.error(error);
      res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: 'Failed to generate AI insights' });
    }
  },
  ...
  async inviteMember(req: Request, res: Response) {
    try {
      ...
      const invite = await teamService.inviteMember(user.organizationId, pid(req), user.id, user.role, { email, roleLabel, message });
  ...
  async requestToJoin(req: Request, res: Response) {
    try {
      ...
      console.error(error);
      res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: 'Failed to request to join team' });
    }
  },
  ...
  async acceptInvite(req: Request, res: Response) {
    try {
      const user = req.user;
      if (!user || !user.organizationId) return res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Unauthorized' });
      const result = await teamService.respondToInvite(user.organizationId, pid(req), pinviteId(req), 'accept', user.id, user.role);
      res.json(result);
    } catch (error: any) {
      console.error(error);
      res.status(StatusCodes.BAD_REQUEST).json({ message: error.message || 'Failed to accept invite' });
    }
  },
  ...
  async declineInvite(req: Request, res: Response) {
    try {
      const user = req.user;
      if (!user || !user.organizationId) return res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Unauthorized' });
      const result = await teamService.respondToInvite(user.organizationId, pid(req), pinviteId(req), 'decline', user.id, user.role);
      res.json(result);
    } catch (error: any) {
      console.error(error);
      res.status(StatusCodes.BAD_REQUEST).json({ message: error.message || 'Failed to decline invite' });
    }
  },
  ...
  async sendMessage(req: Request, res: Response) {
    try {
      const user = req.user;
      if (!user || !user.organizationId) return res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Unauthorized' });
      const message = await teamService.sendMessage(user.organizationId, pid(req), user.id, req.body.message);
      res.status(StatusCodes.CREATED).json(message);
    } catch (error: any) {
      console.error(error);
      res.status(StatusCodes.BAD_REQUEST).json({ message: error.message || 'Failed to send message' });
    }
  },
  ...
  async createTeamTask(req: Request, res: Response) {
    try {
      const user = req.user;
      if (!user || !user.organizationId) return res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Unauthorized' });
      const task = await teamService.createTeamTask(user.organizationId, pid(req), user.id, req.body);
  ...
  async updateTeamTaskStatus(req: Request, res: Response) {
    try {
      ...
      const task = await teamService.updateTeamTaskStatus(user.organizationId, pid(req), ptaskId(req), status, user.id);
      res.json(task);
    } catch (error: any) {
      console.error(error);
      res.status(StatusCodes.BAD_REQUEST).json({ message: error.message || 'Failed to update task' });
    }
  },
  ...
  async createTeamProject(req: Request, res: Response) {
    try {
      const user = req.user;
      if (!user || !user.organizationId) return res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Unauthorized' });
      const project = await teamService.createTeamProject(user.organizationId, pid(req), user.id, req.body);
  ...
  async searchCandidates(req: Request, res: Response) {
    try {
      ...
      const candidates = await teamService.searchCandidates(user.organizationId, q);
      res.json(candidates);
    } catch (error) {
      console.error(error);
      res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: 'Failed to search candidates' });
    }
  },
  ...
  async getCollaborations(req: Request, res: Response) {
    try {
      ...
      console.error(error);
      res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ message: 'Failed to fetch collaborations' });
    }
  },
  ...
  async createCollaboration(req: Request, res: Response) {
    try {
      ...
      if (!toTeamId) return res.status(StatusCodes.BAD_REQUEST).json({ message: 'toTeamId is required' });
      const collab = await teamService.createCollaboration(user.organizationId, pid(req), user.id, user.role, { toTeamId, projectName, message });
      res.status(StatusCodes.CREATED).json(collab);
    } catch (error: any) {
      console.error(error);
      res.status(StatusCodes.BAD_REQUEST).json({ message: error.message || 'Failed to create collaboration request' });
    }
  },
  ...
  async acceptCollaboration(req: Request, res: Response) {
    try {
      const user = req.user;
      if (!user || !user.organizationId) return res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Unauthorized' });
      const result = await teamService.respondToCollaboration(user.organizationId, pcollabId(req), 'accept', user.id, user.role);
      res.json(result);
    } catch (error: any) {
      console.error(error);
      res.status(StatusCodes.BAD_REQUEST).json({ message: error.message || 'Failed to accept collaboration' });
    }
  },
  ...
  async declineCollaboration(req: Request, res: Response) {
    try {
      const user = req.user;
      if (!user || !user.organizationId) return res.status(StatusCodes.UNAUTHORIZED).json({ message: 'Unauthorized' });
      const result = await teamService.respondToCollaboration(user.organizationId, pcollabId(req), 'decline', user.id, user.role);
      res.json(result);
    } catch (error: any) {
      console.error(error);
      res.status(StatusCodes.BAD_REQUEST).json({ message: error.message || 'Failed to decline collaboration' });
    }
  }
};
